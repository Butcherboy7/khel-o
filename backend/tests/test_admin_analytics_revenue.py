...
async def test_revenue_splits_gmv_and_khel_revenue(async_client, db_session):
    ...
    data = resp.json()["data"]
    # The test DB is session-scoped and persists across the whole suite run,
    # so other tests' bookings contribute to these global aggregates too.
    # Assert lower bounds (matching the pattern used in
    # test_admin_analytics_executive.py) rather than exact equality.
